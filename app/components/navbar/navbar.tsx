...
const Navlink: React.FC<{ children: React.ReactNode, href: string }> = ({ children, href }) => {
  return (
    <li className="w-full h-16 grid place-items-center first:border-y border-b border-[rgb(256,256,256,.15)] laptop:h-auto laptop:w-auto laptop:border-0">
      <Link to={href} className="font-sans text-white text-xl tracking-widest laptop:text-sm laptop:text-dark laptop:hover:text-red">
        {children}
      </Link>
    </li>
  )
...
export const Navbar: React.FC = () => {
  ...
  const [sidebar, setSidebar] = useState<{open: boolean}>({ open: true });
  ...
  return (
    <nav className="mt-10 p-container flex justify-between items-center laptop:mt-12">
      ...
      <button className='laptop:hidden' aria-label='open sidebar menu' onClick={openSidebar}>
      ...
      <div
        ...
        <div className="w-full left-0 absolute bottom-12 flex justify-center items-center gap-10 laptop:hidden">
          <SocialLink href="http://wwww.facebook.com" ariaLabel="facebook"
            icon="/assets/icon-facebook.svg"
          />
          <SocialLink href="http://wwww.twitter.com" ariaLabel="twitter"
